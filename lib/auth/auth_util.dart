...
Future<User?> signInOrCreateAccount(
  ...
  try {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
          content: Text(
              'Ошибка авторизации: возможно данный пользователь уже существует')),
    );
    return null;
  }
}
  ...
  try {
    ...
    if (e.code == 'requires-recent-login') {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text(
                'Прошло слишком много времени после последнего входа. Авторизуйтесь ещё раз, перед удалением аккаунта')),
      );
    }
  }
}
